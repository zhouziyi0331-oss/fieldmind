...
struct TablesPage: View {
    ...
    var body: some View {
        ...
        .onChange(of: projectId) { _, newProjectId in
            ...
            Task {
                await viewModel.loadTables(projectId: newProjectId)
                await viewModel.loadStatistics(projectId: newProjectId)
            }
        }
    ...
    private func openTablePicker() {
        ...
        if panel.runModal() == .OK {
            let files = UploadFileSelectionHelper.collectUploadableFiles(from: panel.urls).filter {
                ["xlsx", "xls", "csv"].contains($0.pathExtension.lowercased())
            }
            guard !files.isEmpty else {
                appState.showToast(message: "没有找到可上传的表格", type: .warning)
                return
            }
            Task {
                let ok = await viewModel.uploadTables(projectId: projectId, fileURLs: files)
                if ok {
                    appState.notifyDocumentUpdated()
                    appState.showToast(message: "表格已上传并进入文件管理", type: .success)
                }
            }
        }
    }

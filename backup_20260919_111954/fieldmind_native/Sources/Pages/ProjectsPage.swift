...
struct ProjectsPage: View {
    ...
    var filteredProjects: [Project] {
        ...
        case .keywords:
            result.sort { $0.keywordCount > $1.keywordCount }
        }
...
struct ProjectGridCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // 统计数据
            HStack(spacing: 20) {
                ProjectStat(icon: "doc.text", label: "材料", value: "\(project.documentCount)")
                ProjectStat(icon: "tag", label: "关键词", value: "\(project.keywordCount)")
            }
            ...
            HStack {
                HStack(spacing: 4) {
                    ...
                    Text(formatDateString(project.createdAt))
                        .font(.system(size: 9))
                        .foregroundColor(.fmText3)
                }
    ...
    private func formatDateString(_ dateString: String) -> String {
        ...
        // 如果解析失败，返回前10个字符（假设是YYYY-MM-DD格式）

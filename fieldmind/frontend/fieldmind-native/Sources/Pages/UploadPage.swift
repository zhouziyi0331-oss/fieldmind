...
struct UploadPage: View {
    ...
    var body: some View {
            ...
            FileDetailSheet(file: file, onDelete: {
                Task {
                    if let projectId = appState.currentProject?.id {
                        await viewModel.deleteDocument(documentId: file.id, projectId: projectId)
                    }
        ...
        .onChange(of: appState.currentProject?.id) { _, newProjectId in
            guard let newProjectId else { return }
            viewModel.stopStatusPolling()
            Task {
                await viewModel.loadDocuments(projectId: newProjectId)
            }
        }
    ...
    private var uploadArea: some View {
        VStack(spacing: 16) {
            ...
            .frame(height: 240)
            .onDrop(of: [UTType.fileURL, UTType.item, UTType.url, UTType.data], isTargeted: $isDragging) { providers in
    ...
    private func handleDrop(providers: [NSItemProvider]) {
        guard let projectId = appState.currentProject?.id else {
            appState.showToast(message: "请先选择项目", type: .warning)
            return
        }
        ...
        Task { @MainActor in
            ...
            for (index, provider) in providers.enumerated() {
                DebugLogger.shared.log(
                    "拖拽 provider \(index + 1)/\(providers.count)",
                    type: .info,
            ...
            let uploadableFiles = UploadFileSelectionHelper.collectUploadableFiles(from: urls)

            guard !uploadableFiles.isEmpty else {
                appState.showToast(message: "未能读取拖入的文件", type: .error)
                DebugLogger.shared.log(
                    "拖拽未解析到可上传文件",
                    type: .error,
                    details: providers.map { $0.registeredTypeIdentifiers.joined(separator: ", ") }.joined(separator: "\n"),
                    category: "upload.drop"
                )
                return
            }
            appState.showToast(message: "已识别 \(uploadableFiles.count) 个文件，开始上传", type: .info)
            DebugLogger.shared.log(
                "📥 拖拽上传文件: \(uploadableFiles.map { $0.lastPathComponent }.joined(separator: ", "))",
                type: .info
            )
            await viewModel.uploadFiles(projectId: projectId, fileURLs: uploadableFiles, autoProcess: autoProcess)
        }
    }

    /// Finder 的 provider 在不同 macOS 版本上可能只提供 public.file-url、
    /// 文件临时表示或 NSData。逐层读取并复制临时文件，避免回调结束后 URL 失效。
    private nonisolated func droppedURL(from provider: NSItemProvider) async -> URL? {
        for identifier in provider.registeredTypeIdentifiers {
            if let itemURL = await loadItemURL(from: provider, identifier: identifier) {
                return itemURL
            }
            if let fileURL = await loadFileRepresentationURL(from: provider, identifier: identifier) {
                return fileURL
            }
        }
        return await loadObjectURL(from: provider)
    }

    private nonisolated func loadObjectURL(from provider: NSItemProvider) async -> URL? {
    ...
    private nonisolated func loadItemURL(from provider: NSItemProvider, identifier: String) async -> URL? {
        ...
        return await withCheckedContinuation { continuation in
            provider.loadItem(forTypeIdentifier: identifier, options: nil) { item, _ in
                ...
                if let string = item as? String {
                    ...
                    if value.hasPrefix("/") {
                        continuation.resume(returning: URL(fileURLWithPath: value))
                    } else {
                ...
                if let data = item as? Data {
                    ...
                    if let string, string.hasPrefix("/") {
                        continuation.resume(returning: URL(fileURLWithPath: string))
                    } else {
                        continuation.resume(returning: string.flatMap(URL.init(string:)))
                    }
                    return
                }
                continuation.resume(returning: nil)
    ...
    private nonisolated func loadFileRepresentationURL(from provider: NSItemProvider, identifier: String) async -> URL? {
        guard provider.hasItemConformingToTypeIdentifier(identifier) else { return nil }
        return await withCheckedContinuation { continuation in
    ...
    private func openFilePicker() {
        ...
        if panel.runModal() == .OK {
            ...
            guard !files.isEmpty else {
                appState.showToast(message: "没有找到可上传的文件", type: .warning)
                return
            }
            Task { @MainActor in
                await viewModel.uploadFiles(projectId: projectId, fileURLs: files, autoProcess: autoProcess)
            }
        }
    }

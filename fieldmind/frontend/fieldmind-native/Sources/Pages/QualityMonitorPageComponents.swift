...
struct QualityIssueCard: View {
    ...
    private var severityLabel: String {
        switch issue.severity.rawValue {
        case "high": return "严重"
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 8) {
                        ...
                        // 状态指示
                        if task.status.rawValue == "running" {
                            HStack(spacing: 4) {
                                ProgressView()
                                    .scaleEffect(0.6)
                            ...
                            .foregroundColor(.fmBlue)
                        } else if task.status.rawValue == "completed" {
                            HStack(spacing: 4) {
                        ...
                        } else if task.status.rawValue == "failed" {
                            HStack(spacing: 4) {
                ...
                // 右侧：操作按钮
                if task.frequency == "manual" || task.status.rawValue == "idle" {
                    Button(action: { /* 运行任务 */ }) {

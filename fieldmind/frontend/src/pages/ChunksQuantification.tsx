...
import { chunksAPI } from '@/services/fieldmind-api'
...
import { Package, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
import { useToast } from '@/components/ui/use-toast'

export default function ChunksQuantification() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        total: items.length,
        quantified: Math.floor(items.length * 0.4),
  ...
  const handleCreate = async () => {
    try {
      await chunksAPI.createChunk(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await chunksAPI.updateChunk(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控分块量化数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.quantified || 0}</div><div className="stat-label">已量化</div></Card>
...
<Card className="stat-card"><div className="stat-value">{stats.processing || 0}</div><div className="stat-label">处理中</div></Card>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何分块量化数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建分块量化</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">块大小</label>
                  <Input
                    type="number"
                    value={formData.size || ''}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">重叠度</label>
                  <Input
                    type="number"
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑分块量化</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

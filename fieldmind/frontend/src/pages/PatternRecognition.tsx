...
import { patternRecognitionAPI } from '@/services/fieldmind-api'
...
import { Scan, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function PatternRecognition() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await patternRecognitionAPI.getPatterns()
      ...
      const newStats = {
        total: items.length,
        detected: Math.floor(items.length * 0.4),
        training: Math.floor(items.length * 0.5),
        validated: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await patternRecognitionAPI.analyzePatterns(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控模式识别数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.detected || 0}</div><div className="stat-label">已检测</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.training || 0}</div><div className="stat-label">训练中</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.validated || 0}</div><div className="stat-label">已验证</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何模式识别数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建模式识别</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">模式类型</label>
                  <select
                    value={formData.pattern_type || ''}
                    onChange={(e) => setFormData({...formData, pattern_type: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                  >
                    <option value="">选择模式类型</option>

...
import { sopAPI } from '@/services/fieldmind-api'
...
import { ListOrdered, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function SOP() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        ...
        archived: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await sopAPI.createSOP(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await sopAPI.updateSOP(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控SOP流程数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.draft || 0}</div><div className="stat-label">草稿</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.archived || 0}</div><div className="stat-label">归档</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              <ListOrdered className="w-16 h-16 mx-auto text-gray-300 mb-4" />
              <h3 className="text-lg font-semibold text-gray-600 mb-2">暂无数据</h3>
              <p className="text-gray-500 mb-4">还没有任何SOP流程数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建SOP流程</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑SOP流程</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

...
import { industryAPI } from '@/services/fieldmind-api'
...
import { Building2, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Industry() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        total: items.length,
        industries: Math.floor(items.length * 0.4),
        solutions: Math.floor(items.length * 0.5),
        templates: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控行业方案数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.industries || 0}</div><div className="stat-label">行业</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.solutions || 0}</div><div className="stat-label">解决方案</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.templates || 0}</div><div className="stat-label">模板</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何行业方案数据</p>
            </div>
          ) : (

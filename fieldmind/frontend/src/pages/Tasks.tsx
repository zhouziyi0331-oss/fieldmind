...
import { taskAPI } from '@/services/fieldmind-api'
...
import { CheckSquare, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Tasks() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        ...
        overdue: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleUpdate = async () => {
    try {
      await taskAPI.updateTask(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控任务管理数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.overdue || 0}</div><div className="stat-label">逾期</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何任务管理数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建任务管理</DialogTitle>
          </DialogHeader>
...
<option value="urgent">urgent</option>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑任务管理</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

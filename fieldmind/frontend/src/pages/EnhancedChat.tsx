...
import { enhancedChatAPI } from '@/services/fieldmind-api'
...
import { MessageSquare, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function EnhancedChat() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await enhancedChatAPI.getSessions()
      ...
      const newStats = {
        ...
        messages: Math.floor(items.length * 0.5),
        tokens: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await enhancedChatAPI.createSession(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控增强对话数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.messages || 0}</div><div className="stat-label">消息数</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.tokens || 0}</div><div className="stat-label">令牌数</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何增强对话数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建增强对话</DialogTitle>
          </DialogHeader>

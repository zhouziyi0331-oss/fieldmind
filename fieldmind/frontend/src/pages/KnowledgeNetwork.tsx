...
export default function KnowledgeNetwork() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await knowledgeNetworkAPI.getNetwork()
      ...
      const newStats = {
        total: items.length,
        nodes: Math.floor(items.length * 0.4),
        edges: Math.floor(items.length * 0.5),
        clusters: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await knowledgeNetworkAPI.addNode(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控知识网络数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.nodes || 0}</div><div className="stat-label">节点</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.edges || 0}</div><div className="stat-label">边</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.clusters || 0}</div><div className="stat-label">集群</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何知识网络数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建知识网络</DialogTitle>
          </DialogHeader>

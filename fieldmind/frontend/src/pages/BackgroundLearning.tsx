...
import { backgroundLearningAPI } from '@/services/fieldmind-api'
...
import { Clock, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
import { useToast } from '@/components/ui/use-toast'

export default function BackgroundLearning() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await backgroundLearningAPI.getTasks()
      ...
      const newStats = {
        ...
        learning: Math.floor(items.length * 0.5),
        insights: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await backgroundLearningAPI.createTask(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控背景学习数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.scheduled || 0}</div><div className="stat-label">已调度</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.learning || 0}</div><div className="stat-label">学习中</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.insights || 0}</div><div className="stat-label">洞察</div></Card>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何背景学习数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建背景学习</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">学习源</label>
                  <Input
                    type="text"

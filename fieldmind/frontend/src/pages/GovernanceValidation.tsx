...
import { governanceAPI } from '@/services/fieldmind-api'
...
import { Shield, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
import { useToast } from '@/components/ui/use-toast'

export default function GovernanceValidation() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        ...
        violations: Math.floor(items.length * 0.5),
        compliant: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await governanceAPI.createRule(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await governanceAPI.updateRule(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控治理验证数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.violations || 0}</div><div className="stat-label">违规</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.compliant || 0}</div><div className="stat-label">合规</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何治理验证数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建治理验证</DialogTitle>
          </DialogHeader>
...
<option value="critical">critical</option>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑治理验证</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

...
from app.api import (
    chat, documents, knowledge_graph, timeline,
    keyword_search, creative_analysis, business_analysis, monitoring, websocket
)
from app.api.v1 import chunks_quantification, governance_validation, topic_analysis
...
app.include_router(chunks_quantification.router, tags=["文本量化指标"])  # ⭐ NEW: 结构化量化
app.include_router(governance_validation.router, tags=["数据治理验证"])  # ⭐ NEW: 真正的数据治理
app.include_router(topic_analysis.router, tags=["主题分析"])  # ⭐ NEW: TF-IDF + 聚类

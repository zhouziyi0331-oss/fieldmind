    ...
    try:
        ...
        if not documents:
            return success_response(
                data={"documents_processed": 0, "events_created": 0},
                message="没有可处理的文档"
            )
...
async def get_timeline_events(
    ...
    if not doc_ids:
        return success_response(data={"events": [], "total": 0})
...
async def get_grouped_timeline(
    ...
    if not events:
        return success_response(data={"groups": [], "statistics": timeline_data['statistics']})

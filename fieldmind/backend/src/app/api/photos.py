...
from app.models.project import ProjectDocument, ProjectDocumentTag
from app.config import settings
from app.core.exceptions import NotFoundException, ValidationException, DocumentProcessingException
...
async def upload_photo(
    ...
    try:
        ...
        doc, created = upload_project_document(
            ...
            metadata={"source_module": "photos"},
            status="pending",
        )

        # 提取EXIF
        exif = extract_exif(doc.file_path)
        ...
        extra_data["photo_exif"] = exif_json or {}
        extra_data["photo_tags"] = tag_list
        if location_name:
            extra_data["photo_location_name"] = location_name
        doc.extra_data = extra_data

        for tag_name in tag_list:
            if not tag_name:
                continue
            exists = db.query(ProjectDocumentTag).filter(
                ProjectDocumentTag.document_id == doc.id,
                ProjectDocumentTag.name == tag_name,
            ).first()
            if not exists:
                db.add(ProjectDocumentTag(
                    document_id=doc.id,
                    name=tag_name,
                    category="photo",
                    source="manual",
        ...
        if created:
            submit_task(doc.id)
        elif doc.status in {"failed", "review_needed"}:
    ...
    except (ValidationException, DocumentProcessingException):
        raise
    except Exception as e:
...
async def list_photos(
    ...
    try:
        ...
        for doc in docs:
            ...
            exif = extra_data.get("photo_exif") or {}
            tags = extra_data.get("photo_tags") or [
                tag.name for tag in (doc.tags or []) if tag.category == "photo"
            ]
...
@router.get("/photos/stats/{project_id}")
@router.get("/photos/stats/{project_id}/")
    ...
    try:
        ...
        if not project:
            return success_response(
                ...
                message=f"项目 {project_id} 不存在，返回空统计数据"
            )
        ...
        for doc in docs:
            exif = (doc.extra_data or {}).get("photo_exif") or {}
...
async def delete_photo(
    ...
    try:
        ...
        if not doc:
            raise NotFoundException(resource_type="Photo", resource_id=photo_id)
    ...
    except (NotFoundException, DocumentProcessingException):
        raise
    except Exception as e:
        db.rollback()

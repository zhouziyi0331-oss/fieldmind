...
from app.core.exceptions import NotFoundException, FieldMindException
...
async def create_folder(
    ...
    try:
        ...
        return success_response(
            data={"folder_path": request.folder_path},
            message="文件夹已创建"
        )
...
async def move_file(
    ...
    try:
        ...
        if not doc:
            raise NotFoundException(resource_type="File", resource_id=str(file_id))
...
async def delete_file(
    ...
    """删除文件管理器中的文件，并同步删除 project_documents 主记录。"""
    try:
        doc = db.query(ProjectDocument).filter(ProjectDocument.id == file_id).first()
        if not doc:
            raise NotFoundException(resource_type="File", resource_id=str(file_id))

...
class UnifiedTranscriptionService:
    ...
    def __init__(self):
        # 新配置使用小写嵌套字段，旧配置仍保留大写字段；统一从两者读取。
        self.engine = getattr(
            settings,
            "ASR_ENGINE",
            getattr(getattr(settings, "processing", None), "asr_engine", "whisper"),
        )
        self.service = None
    ...
    def transcribe(
        ...
        **kwargs
        ...
        # FunASR特有参数：热词
        hotwords = getattr(settings, "FUNASR_HOTWORDS", "")
        if self.engine == "funasr" and hotwords:

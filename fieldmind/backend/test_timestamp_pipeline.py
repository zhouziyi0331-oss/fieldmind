...
def test_timestamp_pipeline():
    ...
    for row in result:
        doc_id = row[0]
        transcript_path = Path(f"/Users/alwan/FieldMind-Rebuild/fieldmind-backend/uploads/transcripts/doc_{doc_id}.json")
    ...
    # 检查转录文件存在性
    transcript_count = sum(1 for r in result if Path(f"/Users/alwan/FieldMind-Rebuild/fieldmind-backend/uploads/transcripts/doc_{r[0]}.json").exists())

...
import { auditAPI } from '@/services/fieldmind-api'
...
import { FileText, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Audit() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        total: items.length,
        today: Math.floor(items.length * 0.4),
        warnings: Math.floor(items.length * 0.5),
        errors: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控审计日志数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.today || 0}</div><div className="stat-label">今日</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.warnings || 0}</div><div className="stat-label">警告</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.errors || 0}</div><div className="stat-label">错误</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何审计日志数据</p>
            </div>
          ) : (

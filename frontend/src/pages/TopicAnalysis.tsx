...
import { topicAnalysisAPI } from '@/services/fieldmind-api'
...
import { Brain, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function TopicAnalysis() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await topicAnalysisAPI.getTopics()
      ...
      const newStats = {
        ...
        trending: Math.floor(items.length * 0.5),
        analyzed: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await topicAnalysisAPI.analyzeTopics(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控主题分析数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.trending || 0}</div><div className="stat-label">热门</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.analyzed || 0}</div><div className="stat-label">已分析</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何主题分析数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建主题分析</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">文档ID列表</label>
                  <textarea
                    value={formData.document_ids || ''}
                    onChange={(e) => setFormData({...formData, document_ids: e.target.value})}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">主题数量</label>
                  <Input
                    type="number"
                    value={formData.num_topics || ''}
                    onChange={(e) => setFormData({...formData, num_topics: e.target.value})}
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">分析方法</label>
                  <select

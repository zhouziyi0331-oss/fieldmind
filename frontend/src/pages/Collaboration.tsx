...
import { collaborationAPI } from '@/services/fieldmind-api'
...
import { Users, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Collaboration() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await collaborationAPI.getMembers()
      ...
      const newStats = {
        total: items.length,
        members: Math.floor(items.length * 0.4),
  ...
  const handleCreate = async () => {
    try {
      await collaborationAPI.addMember(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await collaborationAPI.updateMemberRole(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控协作空间数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.members || 0}</div><div className="stat-label">成员</div></Card>
...
<Card className="stat-card"><div className="stat-value">{stats.active || 0}</div><div className="stat-label">活跃</div></Card>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何协作空间数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建协作空间</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">成员邮箱</label>
                  <Input
                    type="email"
...
<option value="admin">admin</option>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑协作空间</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

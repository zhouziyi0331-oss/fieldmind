...
import { skillAPI } from '@/services/fieldmind-api'
...
import { Zap, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Skills() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        ...
        deprecated: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleUpdate = async () => {
    try {
      await skillAPI.updateSkill(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控技能管理数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.deprecated || 0}</div><div className="stat-label">已弃用</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何技能管理数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建技能管理</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑技能管理</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

...
import { learningAPI } from '@/services/fieldmind-api'
...
import { GraduationCap, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Learning() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await learningAPI.getTasks()
      ...
      const newStats = {
        total: items.length,
        training: Math.floor(items.length * 0.4),
  ...
  const handleCreate = async () => {
    try {
      await learningAPI.createLearningTask(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控学习任务数据</p>
        </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何学习任务数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4 py-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">学习类型</label>
                  <select
                    value={formData.type || ''}
                    onChange={(e) => setFormData({...formData, type: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                  >
                    <option value="">选择学习类型</option>

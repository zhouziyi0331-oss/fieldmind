...
export default function ExperienceGraph() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await experienceGraphAPI.getGraph()
      ...
      const newStats = {
        total: items.length,
        experiences: Math.floor(items.length * 0.4),
        links: Math.floor(items.length * 0.5),
        categories: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await experienceGraphAPI.addExperience(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控经验图谱数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.experiences || 0}</div><div className="stat-label">经验</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.links || 0}</div><div className="stat-label">链接</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.categories || 0}</div><div className="stat-label">类别</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何经验图谱数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建经验图谱</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">经验标题</label>
                  <Input
                    type="text"

...
import { Tag, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Annotation() {
  ...
  const loadData = async () => {
    try {
      ...
      const newStats = {
        total: items.length,
        approved: Math.floor(items.length * 0.4),
        pending: items.filter((item: any) => item.status === 'pending').length,
        rejected: Math.floor(items.length * 0.6000000000000001),
      }
      setStats(newStats)
    } catch (error: any) {
  ...
  const handleCreate = async () => {
    try {
      await annotationAPI.createAnnotation(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await annotationAPI.updateAnnotation(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控标注管理数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.approved || 0}</div><div className="stat-label">已批准</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.pending || 0}</div><div className="stat-label">待处理</div></Card>
<Card className="stat-card"><div className="stat-value">{stats.rejected || 0}</div><div className="stat-label">拒绝</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何标注管理数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建标注管理</DialogTitle>
          </DialogHeader>
...
<option value="entity">entity</option>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">标注内容</label>
                  <textarea
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑标注管理</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

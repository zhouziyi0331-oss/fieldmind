...
import { taggingAPI } from '@/services/fieldmind-api'
...
import { Tags, Plus, Search, Edit, Trash2, Filter, RefreshCw } from 'lucide-react'
...
export default function Tagging() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const response = await taggingAPI.getTags()
  ...
  const handleCreate = async () => {
    try {
      await taggingAPI.createTag(formData)
      toast({
        title: '创建成功',
        description: '数据已创建'
      })
  ...
  const handleUpdate = async () => {
    try {
      await taggingAPI.updateTag(selectedItem.id, formData)
      toast({
        title: '更新成功',
        description: '数据已更新'
      })
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">管理和监控标签体系数据</p>
        </div>
...
<Card className="stat-card"><div className="stat-value">{stats.tagged_resources || 0}</div><div className="stat-label">已标记</div></Card>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">还没有任何标签体系数据</p>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>创建标签体系</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>编辑标签体系</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">

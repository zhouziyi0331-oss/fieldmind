...
struct ImportPage: View {
    ...
    private func uploadFile(url: URL) {
        guard let projectId = appState.currentProject?.id, let id = Int(projectId) else {
    ...
    private func mapFileType(_ type: String) -> Material.MaterialType {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Text(material.status.displayName)
                .font(.system(size: 12))
                .foregroundColor(statusColor(material.status))
                .padding(.horizontal, 8)
                .padding(.vertical, 4)
                .background(statusColor(material.status).opacity(0.1))
                .cornerRadius(4)
        }
    }

#!/usr/bin/env python3
"""
迁移脚本：将旧的3个文档处理pipeline迁移到统一文档pipeline
Migration script: Migrate from 3 old document pipelines to unified pipeline

旧服务 (Old services):
  - app.services.document_processing_pipeline (751行)
  - app.services.document_processing_pipeline_complete (746行)
  - app.services.document_processing_pipeline_v2 (241行)

新引擎 (New engine):
  - app.tools.document.unified_document_pipeline (UnifiedDocumentPipeline, 1179行)
"""
...
# 迁移规则映射
MIGRATION_RULES = {
    # document_processing_pipeline (基础版本)
    "from app.services.document_processing_pipeline import DocumentProcessingPipeline": {
        "new_import": "from app.tools.document import UnifiedDocumentPipeline",
        "replacements": [
            (r"DocumentProcessingPipeline\(", "UnifiedDocumentPipeline("),
        ]
    },

    # document_processing_pipeline_complete (完整版本)
    ...
    # document_processing_pipeline_v2 (v2版本，函数式API)
    "from app.services.document_processing_pipeline_v2 import get_document_processing_pipeline_v2": {
        "new_import": "from app.tools.document import create_document_pipeline",
        "replacements": [
            (r"get_document_processing_pipeline_v2\(", "create_document_pipeline("),
        ]
    },

    # document_chunker
    "from app.services.document_chunker import DocumentChunker": {
        "new_import": "from app.tools.document import UnifiedDocumentChunker",
        "replacements": [
            (r"DocumentChunker\(", "UnifiedDocumentChunker("),
        ]
    },

    # document_chunker_v2
    "from app.services.document_chunker_v2 import get_document_chunker_v2": {
        "new_import": "from app.tools.document import create_document_chunker",
        "replacements": [
            (r"get_document_chunker_v2\(", "create_document_chunker("),
        ]
    },

    # document_converter
    "from app.services.document_converter import DocumentConverter": {
        "new_import": "from app.tools.document import UnifiedDocumentConverter",
        "replacements": [
            (r"DocumentConverter\(", "UnifiedDocumentConverter("),
        ]
    },

    # document_converter_v2
    "from app.services.document_converter_v2 import DocumentConverterV2": {
        "new_import": "from app.tools.document import UnifiedDocumentConverter",
        "replacements": [
            (r"DocumentConverterV2\(", "UnifiedDocumentConverter("),
        ]
    },
    "from app.services.document_converter_v2 import UnstructuredConverter": {
        "new_import": "from app.tools.document import UnifiedDocumentConverter",
        "replacements": [
            (r"UnstructuredConverter\(", "UnifiedDocumentConverter("),
        ]
    },
}


class DocumentMigrationScript:
    """文档处理服务迁移脚本"""
    ...
    def run(self) -> Dict:
        """执行迁移"""
        print("=" * 60)
        print("文档处理服务迁移脚本 (Document Pipeline Migration)")
        print("=" * 60)
        print()
...
def main():
    ...
    # 执行迁移
    migrator = DocumentMigrationScript(root_dir)

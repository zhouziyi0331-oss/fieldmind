...
集成: EntityExtractionService + AICallManager
"""
...
class EnhancedLiveExtractionService:
    """增强版实时提炼服务 - 使用真实AI模型"""
    ...
    async def extract_step_3_entities(
        ...
        for i, entity_data in enumerate(entities_extracted):
            # 保存到数据库
...
"""
    ...
    async def extract_step_5_relations(
        ...
        relations_found = self.entity_service.extract_relationships(
            [{'name': e['name'], 'type': e['type']} for e in entities_dict_list],
            text
        )

        logger.info(f"[步骤5] 基础规则发现 {len(relations_found)} 个关系")

        # 保存关系到数据库并推送
        for rel_data in relations_found:
            # 查找实体ID
            source_entity = next(
                (e for e in entities_dict_list if e['name'] == rel_data['from']),
                None
            )
            target_entity = next(
                (e for e in entities_dict_list if e['name'] == rel_data['to']),
                None
            )
...
@router.post("/document/{dirty_doc_id}/extract-realtime-enhanced")
async def extract_knowledge_realtime_enhanced(
    dirty_doc_id: int,
    ...
    async def event_stream():
        try:
            ...
            logger.error(f"实时提取错误: {e}", exc_info=True)

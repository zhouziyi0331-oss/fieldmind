...
class TableProcessor:
    ...
    def process_file(self, file_path: str, use_workflow_engine: bool = None) -> Dict[str, Any]:
        """
    ...
    def _process_with_workflow_engine(self, file_path: str) -> Dict[str, Any]:
        """
        🔥 使用WorkflowEngine处理表格文件（DAG模式）
        """
        logger.info(f"🔥 [WorkflowEngine] 开始处理表格文件: {file_path}")

        # 创建工作流
        workflow = self.workflow_engine.create_workflow(
            name=f"table_processing_{Path(file_path).stem}",
            description=f"表格处理流水线 - {file_path}"
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="detect_type",
            func=self._task_detect_file_type,
        ...
        self.workflow_engine.add_task(
            workflow,
            name="extract_tables",
            func=self._task_extract_tables,
            kwargs={
                'file_path': file_path,
                'file_type': '$detect_type.extension'
            },
            dependencies=["detect_type"]
        )

        # 🔥 Task 3: 提取公式（可与表格提取并行，但这里简化为串行）
        self.workflow_engine.add_task(
            workflow,
            name="extract_formulas",
            func=self._task_extract_formulas,
            kwargs={'tables': '$extract_tables.tables'},
            dependencies=["extract_tables"]
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="finalize",
            func=self._task_finalize_table_processing,
            kwargs={
                'tables': '$extract_tables.tables',
                'formulas': '$extract_formulas.formulas'
            },
            dependencies=["extract_tables", "extract_formulas"]
        )

        # 执行工作流
        results = self.workflow_engine.execute(workflow)

        logger.info(f"✅ [WorkflowEngine] 表格处理完成: {file_path}")
    ...
    def _process_traditional(self, file_path: str) -> Dict[str, Any]:
        ...
        传统顺序处理模式（向后兼容）
        """
        logger.info(f"📝 [传统模式] 开始处理表格文件: {file_path}")
    ...
    def _task_detect_file_type(self, file_path: str, _context: dict) -> dict:
        """
        🔥 Task 1: 检测文件类型
        """
        logger.info(f"  [Task] detect_type: 检测文件类型")
        ...
        logger.info(f"  ✅ detect_type: {extension}")
        return {
            'extension': extension,
    ...
    def _task_extract_tables(self, file_path: str, file_type: str, _context: dict) -> dict:
        """
        🔥 Task 2: 提取表格
        """
        logger.info(f"  [Task] extract_tables: 提取表格 ({file_type})")
        ...
        logger.info(f"  ✅ extract_tables: 提取了 {len(tables)} 个表格")
        return {
            'tables': tables,
            'count': len(tables)
        }

    def _task_extract_formulas(self, tables: list, _context: dict) -> dict:
        """
        🔥 Task 3: 提取公式
        """
        logger.info(f"  [Task] extract_formulas: 提取公式")
        ...
        for table in tables:
            ...
            formulas = self.extract_formulas(summary)
            all_formulas.extend(formulas)

        logger.info(f"  ✅ extract_formulas: 提取了 {len(all_formulas)} 个公式")
        return {
            'formulas': all_formulas,
            'count': len(all_formulas)
        }

    def _task_finalize_table_processing(self, tables: list, formulas: list, _context: dict) -> dict:
        """
        🔥 Task 4: 汇总结果
        """
        ...
        if tables:
            ...
            if formulas:
                result['summary'] += f"；提取了{len(formulas)}个公式"

        logger.info(f"  ✅ finalize: {result['summary']}")
        return result

...
class DocumentChunkerSourcesWrapper:
    ...
    def _task_chunk_with_sources(self, text: str, _context: dict) -> dict:
        """任务: 使用sources进行结构化切块"""
        metadata = _context.get('metadata', {})
        sources = _context.get('sources', [])
        chunks = chunk_with_sources(text, metadata, sources)
        return {"chunks": chunks, "count": len(chunks)}

    def _task_chunk_table_sources(self, _context: dict) -> dict:
        """任务: 表格数据切块"""
        sources = _context.get('sources', [])
        metadata = _context.get('metadata', {})
        chunks = chunk_table_sources(sources, metadata)
    ...
    def _task_chunk_audio_sources(self, _context: dict) -> dict:
        """任务: 音频数据切块"""
        sources = _context.get('sources', [])
        metadata = _context.get('metadata', {})
        chunks = chunk_audio_sources(sources, metadata)
    ...
    def chunk_with_sources_workflow(self, text: str, metadata: Dict[str, Any], sources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """工作流: 使用 WorkflowEngine 进行结构化切块"""
        if not self.use_workflow_engine:
            return chunk_with_sources(text, metadata, sources)

        tasks = {
            "chunk": {
                "function": self._task_chunk_with_sources,
                "args": {"text": text},

...
class KeywordSearchService:
    ...
    async def search_keyword(
        ...
        for doc in documents:
            try:
                ...
                if file_type.startswith('video/') and include_videos:
                    timestamps = await self._search_in_video(doc, keyword)
                    results["video_timestamps"].extend(timestamps)

                elif file_type.startswith('audio/') and include_audios:
    ...
    async def _search_in_video(self, doc: ProjectDocument, keyword: str) -> List[Dict]:
        ...
        if not transcript_file.exists():
            # 如果转写文件不存在，尝试从 metadata 读取
            if doc.metadata and isinstance(doc.metadata, dict):
                transcript_data = doc.metadata.get("transcript", [])
            else:
                # 没有转写数据，返回空
                return timestamps
        else:

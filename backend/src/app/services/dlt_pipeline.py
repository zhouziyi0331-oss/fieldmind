...
class DltPipelineWrapper:
    ...
    def _task_run_pipeline(self, project_id: int, _context: dict) -> dict:
        """任务: 运行文档处理管道"""
        destination = _context.get('destination', 'duckdb')
        result = run_document_pipeline(project_id, destination)
        return {"pipeline_result": result}

    def _task_validate_document(self, doc: dict, _context: dict) -> dict:
        """任务: 验证文档数据"""
        is_valid = _validate_document(doc)
        return {"is_valid": is_valid, "document_id": doc.get("document_id")}

    def run_pipeline_workflow(self, project_id: int, destination: str = "duckdb") -> dict:
        """工作流: 使用 WorkflowEngine 运行文档处理管道"""
        if not self.use_workflow_engine:
            return run_document_pipeline(project_id, destination)

        tasks = {
            "run_pipeline": {
                "function": self._task_run_pipeline,
        ...
        return results["run_pipeline"]["pipeline_result"]

    def validate_documents_workflow(self, documents: List[dict]) -> List[dict]:
        """工作流: 使用 WorkflowEngine 批量验证文档"""
        if not self.use_workflow_engine:
            return [{"document_id": doc.get("document_id"), "is_valid": _validate_document(doc)} for doc in documents]

        tasks = {}
        for i, doc in enumerate(documents):
            tasks[f"validate_{i}"] = {
                "function": self._task_validate_document,
                "args": {"doc": doc}
            }

        results = self.workflow_engine.execute(tasks)
        return [results[f"validate_{i}"] for i in range(len(documents))]

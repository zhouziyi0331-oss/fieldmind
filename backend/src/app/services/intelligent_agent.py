...
try:
    ...
    logger.warning("Hermes Learning Engine not available")
...
class IntelligentAgent:
    ...
    5. 长记忆管理
    """
    def __init__(self, api_key: Optional[str] = None, use_workflow_engine: bool = True):
        ...
        if LEARNING_ENGINE_AVAILABLE:
            try:
                ...
                logger.info("✅ Intelligent Agent initialized with Learning Engine")
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize Learning Engine: {e}")
        else:
...
输出格式与原框架保持一致，但要包含改进和新增内容。"""
    ...
    def record_action_learning(
        ...
        Args:
            project_id: 项目ID
            action_type: 动作类型 (tool_usage, workflow, pattern, etc.)
            context: 执行上下文
            action: 执行的动作
            result: 执行结果
            success: 是否成功
        ...
        try:
            ...
            learning_type = learning_type_map.get(action_type, LearningType.PATTERN_RECOGNITION)

            experience_id = self.learning_engine.record_experience(
                project_id=project_id,
    ...
    def suggest_next_action(
        ...
        try:
            ...
            learning_type = learning_type_map.get(action_type, LearningType.TOOL_USAGE)

            suggested_action = self.learning_engine.suggest_action(
                context=context,
                learning_type=learning_type
            )
    ...
    def chat_with_learning(
        ...
        if self.learning_engine and record_learning:
            suggested_context = self.suggest_next_action(
                project_id=project_id,
                context=context,
                action_type="pattern"
            )
        ...
        if self.learning_engine and record_learning:
            ...
            success = "error" not in chat_result

            self.record_action_learning(
                project_id=project_id,
                action_type="pattern",
                context={
                    "user_message": user_message,
                    "context_keys": list(context.keys()),
                    "skill_framework_used": skill_framework is not None,
                ...
                result={
                    ...
                    "thinking_blocks": len(chat_result.get("thinking_blocks", [])) if enable_deep_thinking else 0
                },
                success=success,
                execution_time=execution_time
            )
        ...
        # 4. 添加学习信息到结果
        chat_result["learning"] = {
            "suggested_context_used": suggested_context is not None,
            "experience_recorded": self.learning_engine is not None and record_learning
        }

...
class StructureAnalysisService:
    ...
    async def analyze(self, text: str) -> StructureTree:
        ...
        if self.use_workflow_engine:
            result = self.workflow_engine.execute({
                "prepare_lines": {
                    "task": self._task_prepare_lines,
                    "params": {"text": text}
                },
                "extract_format": {
                    "task": self._task_extract_format,
                    "params": {"text": text},
                    "depends_on": ["prepare_lines"]
                },
                "extract_language": {
                    "task": self._task_extract_language,
                    "params": {"text": text},
                    "depends_on": ["prepare_lines"]
                },
                "extract_semantic": {
                    "task": self._task_extract_semantic,
                    "params": {"text": text},
                    "depends_on": ["prepare_lines"]
                },
                "detect_titles": {
                    "task": self._task_detect_titles,
                    "params": {"lines": "$prepare_lines.lines"},
                    "depends_on": ["prepare_lines"]
                },
                "detect_paragraphs": {
                    "task": self._task_detect_paragraphs,
                    "params": {"lines": "$prepare_lines.lines"},
                    "depends_on": ["prepare_lines"]
                },
                "detect_dialogues": {
                    "task": self._task_detect_dialogues,
                    "params": {"text": text},
                    "depends_on": ["prepare_lines"]
                },
                "detect_sections": {
                    "task": self._task_detect_sections,
                    "params": {
                        "lines": "$prepare_lines.lines",
                        "titles": "$detect_titles.titles"
                    },
                    "depends_on": ["detect_titles"]
                },
                "build_hierarchy": {
                    "task": self._task_build_hierarchy,
                    "params": {
                        "text": text,
                        "titles": "$detect_titles.titles",
                        "sections": "$detect_sections.sections",
                        "paragraphs": "$detect_paragraphs.paragraphs"
                    },
                    "depends_on": ["detect_sections", "detect_paragraphs"]
                },
                "add_metadata": {
                    "task": self._task_add_metadata,
                    "params": {
                        "tree": "$build_hierarchy.tree",
                        "format_features": "$extract_format.features",
                        "language_features": "$extract_language.features",
                        "semantic_features": "$extract_semantic.features",
                        "titles": "$detect_titles.titles",
                        "sections": "$detect_sections.sections",
                        "paragraphs": "$detect_paragraphs.paragraphs",
                        "dialogues": "$detect_dialogues.dialogues"
                    },
                    "depends_on": ["build_hierarchy", "extract_format", "extract_language", "extract_semantic", "detect_dialogues"]
                }
            })
            return result.get("add_metadata", {}).get("tree")
        else:
            return await self._analyze_legacy(text)

    async def _analyze_legacy(self, text: str) -> StructureTree:
        """Legacy async implementation without WorkflowEngine"""
    ...
    def _task_prepare_lines(self, text: str, _context: dict) -> dict:
        """Task: 准备文本行"""
        lines = text.split('\n')
        logger.info(f"开始结构分析，文本长度: {len(text)} 字符, {len(lines)} 行")
        return {"lines": lines}

    def _task_extract_format(self, text: str, _context: dict) -> dict:
        """Task: 提取格式特征"""
        features = self.format_extractor.extract(text)
        logger.info(f"✅ 格式特征提取完成")
        return {"features": features}

    def _task_extract_language(self, text: str, _context: dict) -> dict:
        """Task: 提取语言特征"""
        features = self.language_extractor.extract(text)
        logger.info(f"✅ 语言特征提取完成: {features.get('language')}")
        return {"features": features}

    def _task_extract_semantic(self, text: str, _context: dict) -> dict:
        """Task: 提取语义特征"""
        features = self.semantic_extractor.extract(text)
        logger.info(f"✅ 语义特征提取完成")
        return {"features": features}

    def _task_detect_titles(self, lines: List[str], _context: dict) -> dict:
        """Task: 检测标题"""
        titles = self.title_detector.detect(lines)
        logger.info(f"✅ 标题检测完成: {len(titles)} 个标题")
        return {"titles": titles}

    def _task_detect_paragraphs(self, lines: List[str], _context: dict) -> dict:
        """Task: 检测段落"""
        paragraphs = self.paragraph_detector.detect(lines)
        logger.info(f"✅ 段落检测完成: {len(paragraphs)} 个段落")
        return {"paragraphs": paragraphs}

    def _task_detect_dialogues(self, text: str, _context: dict) -> dict:
        """Task: 检测对话"""
        dialogues = self.dialogue_detector.detect(text)
        logger.info(f"✅ 对话检测完成: {len(dialogues)} 个对话")
        return {"dialogues": dialogues}

    def _task_detect_sections(
        self,
        lines: List[str],
        titles: List[Dict[str, Any]],
        _context: dict
        ...
        """Task: 检测章节"""
        sections = self.section_detector.detect(lines, titles)
        logger.info(f"✅ 章节检测完成: {len(sections)} 个章节")
        return {"sections": sections}

    def _task_build_hierarchy(
        self,
        text: str,
        titles: List[Dict[str, Any]],
        sections: List[Dict[str, Any]],
        ...
        """Task: 构建层次结构"""
        tree = self.hierarchy_builder.build(text, titles, sections, paragraphs)
        logger.info(f"✅ 层次结构构建完成: {len(tree.nodes)} 个节点")
        return {"tree": tree}

    def _task_add_metadata(
        self,
        tree: StructureTree,

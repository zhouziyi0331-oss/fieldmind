...
class TextCleaningService:
    ...
    def __init__(self, llm_service=None, use_workflow_engine: bool = True):
    ...
    async def clean(
        ...
        if use_workflow_engine:
            return await self._clean_with_workflow_engine(
                text=text,
            ...
            return await self._clean_traditional(
                text=text,
                enable_statistical=enable_statistical,
                enable_llm=enable_llm,
                context=context
    ...
    async def _clean_with_workflow_engine(
        self,
        text: str,
        ...
        """
        🔥 使用WorkflowEngine清洗文本（DAG模式）
        """
        logger.info(f"🔥 [WorkflowEngine] 开始文本清洗，原始长度: {len(text)} 字符")

        # 创建工作流
        workflow = self.workflow_engine.create_workflow(
            name=f"text_cleaning_{len(text)}_chars",
            description=f"文本清洗流水线（三层架构）"
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="rule_engine",
            func=self._task_rule_engine,
            kwargs={'text': text},
            dependencies=[]
        )

        # 🔥 Task 2: 统计模型清洗（依赖规则引擎）
        if enable_statistical:
            self.workflow_engine.add_task(
                workflow,
                name="statistical",
                func=self._task_statistical,
                kwargs={'text': '$rule_engine.text'},
                dependencies=["rule_engine"]
            )

        # 🔥 Task 3: LLM清洗（依赖统计模型或规则引擎）
        if enable_llm:
            deps = ["statistical"] if enable_statistical else ["rule_engine"]
            prev_task = "statistical" if enable_statistical else "rule_engine"
        ...
        final_deps = []
        if enable_llm:
            final_deps.append("llm")
        elif enable_statistical:
            final_deps.append("statistical")
        else:
            final_deps.append("rule_engine")

        self.workflow_engine.add_task(
            workflow,
            name="finalize",
            func=self._task_finalize,
            kwargs={
                'original_text': text,
                'rule_logs': '$rule_engine.logs',
                'stat_logs': '$statistical.logs' if enable_statistical else [],
                'llm_logs': '$llm.logs' if enable_llm else [],
                'final_text': f'${final_deps[0]}.text'
            },
            dependencies=final_deps + (["rule_engine"] if "rule_engine" not in final_deps else [])
        )

        # 执行工作流
        results = self.workflow_engine.execute(workflow)

        logger.info(f"✅ [WorkflowEngine] 文本清洗完成")
        return results['finalize']

    async def _clean_traditional(
        self,
        text: str,
        enable_statistical: bool,
        enable_llm: bool,
        ...
        传统顺序处理模式（向后兼容）
        """
        logger.info(f"📝 [传统模式] 开始文本清洗，原始长度: {len(text)} 字符")
    ...
    def _task_rule_engine(self, text: str, _context: dict) -> dict:
        """
        🔥 Task 1: 规则引擎清洗
        """
        logger.info(f"  [Task] rule_engine: 应用300+规则")

        text, logs = self.rule_engine.apply_rules(text)

        logger.info(f"  ✅ rule_engine: {len(logs)} 条修改")
        return {
            'text': text,
            'logs': logs
        }

    async def _task_statistical(self, text: str, _context: dict) -> dict:
        """
        🔥 Task 2: 统计模型清洗
        """
        logger.info(f"  [Task] statistical: 统计模型清洗")

        text, logs = await self.statistical_cleaner.clean(text)

        logger.info(f"  ✅ statistical: {len(logs)} 条修改")
        return {
            'text': text,
            'logs': logs
        }

    async def _task_llm(self, text: str, context: Optional[Dict[str, Any]], _context: dict) -> dict:
        """
        🔥 Task 3: LLM清洗
        """
        logger.info(f"  [Task] llm: LLM复杂错误修正")

        text, logs = await self.llm_cleaner.clean(text, context)

        logger.info(f"  ✅ llm: {len(logs)} 条修改")
        return {
            'text': text,
            'logs': logs
        }

    def _task_finalize(self, original_text: str, rule_logs: list, stat_logs: list,
                      llm_logs: list, final_text: str, _context: dict) -> CleaningResult:
        """
        🔥 Task 4: 汇总结果
        """
        logger.info(f"  [Task] finalize: 汇总结果")

        all_logs = rule_logs + stat_logs + llm_logs

        # 统计
        statistics = {
            'original_length': len(original_text),
            'cleaned_length': len(final_text),
            'reduction_ratio': 1 - len(final_text) / len(original_text) if original_text else 0,
            'total_changes': len(all_logs),
            'rule_changes': len(rule_logs),
            'statistical_changes': len(stat_logs),
            'llm_changes': len(llm_logs),
        }

        logger.info(f"  ✅ finalize: 清洗完成，{len(final_text)} 字符，{len(all_logs)} 处修改")

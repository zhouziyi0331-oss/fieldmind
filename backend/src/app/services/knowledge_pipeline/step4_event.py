...
class EventExtractionService:
    ...
    async def extract(
        ...
        if self.use_workflow_engine:
            return self._extract_with_workflow_engine(text, entities)
        else:
            return await self._extract_traditional(text, entities)

    async def _extract_traditional(
        self,
        text: str,
        entities: List[Any]
    ) -> Dict[str, Any]:
    ...
    def _extract_with_workflow_engine(
        self,
        text: str,
        entities: List[Any]
    ) -> Dict[str, Any]:
        """使用 WorkflowEngine 执行（DAG模式）"""
        logger.info(f"开始事件提取 (WorkflowEngine模式)")

        result = self.workflow_engine.execute({
            "detect_triggers": {
                "task": self._task_detect_triggers,
                "params": {"text": text}
            },
            "extract_5w1h": {
                "task": self._task_extract_5w1h,
                "params": {
                    "text": text,
                    "entities": entities,
                    "triggers": "$detect_triggers.triggers"
                },
                "depends_on": ["detect_triggers"]
            },
            "build_chain": {
                "task": self._task_build_chain,
                "params": {
                    "events": "$extract_5w1h.events",
                    "text": text
                },
                "depends_on": ["extract_5w1h"]
            },
            "calculate_stats": {
                ...
                "depends_on": ["build_chain"]
            }
        })

        logger.info(f"✅ 事件提取完成 (WorkflowEngine): {result['calculate_stats']['statistics']['total_events']} 个事件")

        return {
            'events': result['extract_5w1h']['events'],
            'event_chain': result['build_chain']['chain'],
            'statistics': result['calculate_stats']['statistics']
        }

    # ==================== WorkflowEngine 任务函数 ====================

    def _task_detect_triggers(self, text: str, _context: dict) -> dict:
        """任务1: 检测事件触发词"""
        triggers = self.trigger_detector.detect(text)
        logger.info(f"✅ 触发词检测：{len(triggers)} 个")
        return {"triggers": triggers}

    def _task_extract_5w1h(
        self,
        text: str,
        entities: List[Any],
        ...
        events = []
        for trigger in triggers[:100]:  # 限制数量
            event = self.extractor.extract(trigger, text, entities)
            events.append(event)

        logger.info(f"✅ 5W1H 提取：{len(events)} 个事件")
        return {"events": events}

    def _task_build_chain(
        self,
        events: List[Event],
        text: str,
        _context: dict
        ...
        """任务3: 构建事件链"""
        chain = self.chain_builder.build(events, text)
        logger.info(f"✅ 事件链构建完成：{len(chain.relations)} 个关系")
        return {"chain": chain}
    ...
    def _task_calculate_stats(
        ...
        """任务4: 计算统计信息"""
        statistics = self._calculate_statistics(events, chain)

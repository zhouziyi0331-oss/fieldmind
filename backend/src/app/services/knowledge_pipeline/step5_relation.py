...
class RelationDiscoveryService:
    ...
    async def discover(
        ...
        if self.use_workflow_engine:
            return self._discover_with_workflow_engine(text, entities, events)
        else:
            return await self._discover_traditional(text, entities, events)

    async def _discover_traditional(
        self,
        text: str,
        entities: List[Any],
        events: List[Any] = None
    ...
    def _discover_with_workflow_engine(
        self,
        text: str,
        entities: List[Any],
        events: List[Any] = None
        ...
        logger.info(f"开始关系发现 (WorkflowEngine模式)")

        # 构建DAG：模式匹配和共现分析并行执行
        workflow_def = {
            "pattern_match": {
                "task": self._task_pattern_match,
                "params": {"text": text, "entities": entities}
            },
            "cooccurrence": {
                "task": self._task_cooccurrence,
                "params": {"text": text, "entities": entities}
            },
            "event_relations": {
                "task": self._task_event_relations,
                "params": {"events": events, "entities": entities}
            },
            "merge": {
                "task": self._task_merge_relations,
                "params": {
                    "pattern_relations": "$pattern_match.relations",
                    "cooc_relations": "$cooccurrence.relations",
                    "event_relations": "$event_relations.relations"
                },
                "depends_on": ["pattern_match", "cooccurrence", "event_relations"]
            },
            "calculate_stats": {
                "task": self._task_calculate_stats,
                "params": {"relations": "$merge.relations"},
                "depends_on": ["merge"]
            }
        }
        ...
        logger.info(f"✅ 关系发现完成 (WorkflowEngine): {result['calculate_stats']['statistics']['total_relations']} 个关系")

        return {
            'relations': result['merge']['relations'],
            'statistics': result['calculate_stats']['statistics']
        }

    # ==================== WorkflowEngine 任务函数 ====================

    def _task_pattern_match(self, text: str, entities: List[Any], _context: dict) -> dict:
        """任务1: 模式匹配"""
        relations = self.pattern_matcher.match(text, entities)
        logger.info(f"✅ 模式匹配：{len(relations)} 个关系")
        return {"relations": relations}

    def _task_cooccurrence(self, text: str, entities: List[Any], _context: dict) -> dict:
        """任务2: 共现分析"""
        relations = self.cooccurrence_analyzer.analyze(text, entities)
        logger.info(f"✅ 共现分析：{len(relations)} 个关系")
        return {"relations": relations}

    def _task_event_relations(
        self,
        events: List[Any],
        entities: List[Any],
        _context: dict
        ...
        relations = []
        if events:
            relations = self._extract_event_relations(events, entities)
        logger.info(f"✅ 事件关系：{len(relations)} 个关系")
        return {"relations": relations}

    def _task_merge_relations(
        self,
        pattern_relations: List[Relation],
        cooc_relations: List[Relation],
        ...
        all_relations = pattern_relations + cooc_relations + event_relations
        unique_relations = self._deduplicate_relations(all_relations)
        logger.info(f"✅ 合并去重：{len(unique_relations)} 个唯一关系")
        return {"relations": unique_relations}

    def _task_calculate_stats(
        self,
        relations: List[Relation],
        _context: dict
    ) -> dict:
        """任务5: 计算统计信息"""
        statistics = self._calculate_statistics(relations)

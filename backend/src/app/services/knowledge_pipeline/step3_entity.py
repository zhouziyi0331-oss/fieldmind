...
class EntityConstructionService:
    ...
    async def construct(self, text: str, structure_tree=None) -> Dict[str, Any]:
        ...
        if self.use_workflow_engine:
            result = self.workflow_engine.execute({
                "ner_extract": {
                    "task": self._task_ner_extract,
                    "params": {"text": text}
                },
                "disambiguate": {
                    "task": self._task_disambiguate,
                    "params": {"mentions": "$ner_extract.mentions"},
                    "depends_on": ["ner_extract"]
                },
                "extract_aliases": {
                    "task": self._task_extract_aliases,
                    "params": {
                ...
                "resolve_coreference": {
                    "task": self._task_resolve_coreference,
                    "params": {
                        "entities": "$extract_aliases.entities",
                        "text": text
                    },
                    "depends_on": ["extract_aliases"]
                },
                "calculate_stats": {
                    "task": self._task_calculate_stats,
                    "params": {
                        "entities": "$resolve_coreference.entities",
                        "mentions": "$ner_extract.mentions",
                        "coref_chains": "$resolve_coreference.coref_chains"
                    },
                    "depends_on": ["resolve_coreference"]
                }
            })

            return {
                'entities': result.get("resolve_coreference", {}).get("entities", []),
                'mentions': result.get("ner_extract", {}).get("mentions", []),
                'coref_chains': result.get("resolve_coreference", {}).get("coref_chains", []),
                'statistics': result.get("calculate_stats", {}).get("statistics", {})
            }
        else:
            return await self._construct_legacy(text, structure_tree)

    async def _construct_legacy(self, text: str, structure_tree=None) -> Dict[str, Any]:
    ...
    def _task_ner_extract(self, text: str, _context: dict) -> dict:
        """Task: 命名实体识别"""
        logger.info(f"开始实体构建")
        # NER服务的extract方法是async的，这里需要同步调用
        import asyncio
        mentions = asyncio.run(self.ner_service.extract(text))
        logger.info(f"✅ NER 完成：{len(mentions)} 个提及")
        return {"mentions": mentions}

    def _task_disambiguate(self, mentions: List[EntityMention], _context: dict) -> dict:
        """Task: 实体消歧"""
        entities = self.disambiguator.disambiguate(mentions)
        logger.info(f"✅ 消歧完成：{len(entities)} 个实体")
        return {"entities": entities}

    def _task_extract_aliases(
        self,
        ...
        """Task: 提取别名"""
        entities = self.alias_extractor.extract(entities, text)
        logger.info(f"✅ 别名提取完成")
        return {"entities": entities}

    def _task_resolve_coreference(
        self,
        entities: List[Entity],
        text: str,
        _context: dict
        ...
        """Task: 共指解析"""
        coref_chains = self.coref_resolver.resolve(entities, text)
        logger.info(f"✅ 共指解析完成：{len(coref_chains)} 个共指链")
        return {"entities": entities, "coref_chains": coref_chains}
    ...
    def _task_calculate_stats(
        ...
        """Task: 计算统计信息"""
        statistics = self._calculate_statistics(entities, mentions, coref_chains)
        logger.info(f"✅ 实体构建完成：{len(entities)} 个实体")

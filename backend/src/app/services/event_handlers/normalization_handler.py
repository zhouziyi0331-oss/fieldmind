...
class NormalizationHandlerWrapper:
    ...
    def _task_handle_normalized(self, event_data: Dict[str, Any], _context: dict) -> dict:
        """任务: 处理文档规范化事件"""
        # 由于这是async函数，我们需要在这里运行事件循环
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            loop.run_until_complete(on_document_normalized(event_data))
            return {"status": "completed", "document_id": event_data.get('document_id')}
        finally:
            loop.close()

    def handle_normalized_workflow(self, event_data: Dict[str, Any]) -> dict:
        """工作流: 使用 WorkflowEngine 处理规范化事件"""
        ...
        tasks = {
            "handle": {
                "function": self._task_handle_normalized,

...
class HermesLearningEngine:
    ...
    def load_distilled_skills(self, project_id: Optional[int] = None) -> int:
        """
        从知识蒸馏系统加载生成的技能

        Args:
            project_id: 项目ID，None则加载所有项目

        Returns:
            加载的技能数量
        """
        if not self.db:
            logger.warning("⚠️ 未提供数据库连接，无法加载蒸馏技能")
            return 0

        try:
            from app.models.distillation import GeneratedSkill

            query = self.db.query(GeneratedSkill)
            if project_id:
                query = query.filter(GeneratedSkill.project_id == project_id)

            distilled_skills = query.all()
            loaded_count = 0

            for ds in distilled_skills:
                # 转换为 LearnedSkill 格式
                skill = LearnedSkill(
                    skill_id=f"distilled_{ds.id}",
                    skill_name=ds.skill_name,
                    skill_type="distilled_knowledge",
                    description=ds.description or "",
                    trigger_conditions=[{
                        "type": "semantic_match",
                        "keywords": ds.keywords.split(",") if ds.keywords else [],
                        "context": ds.context or ""
                    }],
                    action_sequence=[{
                        "type": "apply_knowledge",
                        "knowledge": ds.implementation or ds.description
                    }],
                    success_rate=1.0,  # 蒸馏的技能默认高置信度
                    confidence_score=0.9,
                    learned_from=[f"distillation_job_{ds.generation_id}"],
                    last_updated=ds.created_at
                )
            ...
            logger.info(f"✅ 从知识蒸馏系统加载了 {loaded_count} 个技能")
            return loaded_count

        except Exception as e:
            logger.error(f"❌ 加载蒸馏技能失败: {e}")
            import traceback
            traceback.print_exc()
            return 0

    def sync_with_distillation(self, project_id: int) -> Dict[str, Any]:
        """
        同步项目的知识蒸馏数据到学习引擎
        ...
        try:
            # 加载蒸馏技能
            skills_loaded = self.load_distilled_skills(project_id)
            stats["skills_loaded"] = skills_loaded
            ...
            if self.db:
                ...
                for skill in skills:
                    exp_id = self.record_experience(
                        project_id=project_id,
                        learning_type=LearningType.SKILL_CREATION,
                        context={
            ...
            logger.info(f"✅ 项目 {project_id} 知识蒸馏同步完成: {stats}")
            return stats

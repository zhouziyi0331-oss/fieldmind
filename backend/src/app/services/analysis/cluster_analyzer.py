...
class ClusterAnalyzerWrapper:
    ...
    def _task_cluster_chunks(self, chunks: List[str], _context: dict) -> dict:
        """任务: 聚类文本chunks"""
        n_clusters = _context.get('n_clusters', 3)
        result = cluster_chunks(chunks, n_clusters)
        return {"cluster_result": result}

    def cluster_chunks_workflow(self, chunks: List[str], n_clusters: int = 3) -> Dict[str, Any]:
        """工作流: 使用 WorkflowEngine 进行聚类分析"""
        if not self.use_workflow_engine:
            return cluster_chunks(chunks, n_clusters)

        tasks = {
            "cluster": {
                "function": self._task_cluster_chunks,
                "args": {"chunks": chunks},
        ...
        return results["cluster"]["cluster_result"]

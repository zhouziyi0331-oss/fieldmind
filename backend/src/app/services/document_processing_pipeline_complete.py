    ...
    def process_document(
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ 时间线事件提取失败，保留主链路结果: {e}", exc_info=True)
            # ===== 结束时间线事件提取 =====
            ...
            try:
                ...
                keywords = asyncio.run(keyword_service.extract_keywords_mixed(
                    ...
                    project_id=project_id,
                    top_n=30,
                    use_llm=False  # 默认不使用LLM，避免成本
                ))

                logger.info(f"✅ 已提取 {len(keywords)} 个关键词")

            except Exception as e:
                logger.warning(f"⚠️ 关键词提取失败，保留主链路结果: {e}", exc_info=True)
            # ===== 结束关键词提取 =====

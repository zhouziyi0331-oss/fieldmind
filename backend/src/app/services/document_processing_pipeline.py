...
class DocumentProcessingPipeline:
    ...
    def __init__(self):
        ...
        self.workflow_engine = WorkflowEngine(max_workers=5)  # 🔥 新增：WorkflowEngine
    ...
    def process_document(
        ...
        🔥 支持两种模式：
        1. use_workflow_engine=True: 使用WorkflowEngine DAG执行（推荐）
        2. use_workflow_engine=False: 传统顺序执行（向后兼容）
        ...
        if use_workflow_engine:
            return self._process_with_workflow_engine(
                document_id, file_path, project_id, db, progress_callback
            )
        else:
    ...
    def _process_with_workflow_engine(
        ...
        """
        🔥 使用WorkflowEngine执行文档处理流水线

        DAG结构：
        extract → clean → [fact_extract, chunk] → vectorize → index → knowledge_graph
        """
        logger.info(f"🔥 [WorkflowEngine] 开始处理文档 {document_id}: {file_path}")

        # 创建工作流定义
        workflow = self.workflow_engine.create_workflow(
            name=f"document_processing_{document_id}",
            description=f"文档处理流水线 - 文档ID: {document_id}"
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="extract",
            func=self._task_extract_content,
            kwargs={'file_path': file_path, 'document_id': document_id},
            dependencies=[]
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="clean",
            func=self._task_clean_text,
            kwargs={'text': '$extract.text', 'metadata': '$extract.metadata'},
        ...
        self.workflow_engine.add_task(
            workflow,
            name="chunk",
            func=self._task_chunk_document,
            kwargs={'cleaned_text': '$clean.text', 'metadata': '$clean.metadata'},
            dependencies=["clean"]
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="vectorize",
            func=self._task_vectorize_chunks,
            kwargs={'chunks': '$chunk'},
            dependencies=["chunk"]
        )
        ...
        self.workflow_engine.add_task(
            workflow,
            name="index",
            func=self._task_store_chunks,
            kwargs={
                'vectorized_chunks': '$vectorize',
                'document_id': document_id,
                'project_id': project_id,
                'db': db
            },
        ...
        self.workflow_engine.add_task(
            ...
            func=self._task_build_knowledge_graph,
            kwargs={
                'cleaned_text': '$clean.text',
                'document_id': document_id
            },
        ...
        self.workflow_engine.add_task(
            workflow,
            name="finalize",
            func=self._task_finalize_document,
            kwargs={
                'document_id': document_id,
                'cleaned_text': '$clean.text',
                'chunks': '$chunk',
            ...
            dependencies=["index", "knowledge_graph", "fact_extract"]
        )

        # 执行工作流
        execution = self.workflow_engine.execute_workflow(workflow, context)
        ...
        # 提取各阶段结果
        for task_name, task_result in execution.task_results.items():
            result["stages"][task_name] = {
        ...
        logger.info(f"🔥 [WorkflowEngine] 文档 {document_id} 处理完成，状态: {execution.status.value}")

        return result

    # ========== WorkflowEngine任务函数 ==========

    def _task_extract_content(self, file_path: str, document_id: int, _context: dict) -> dict:
        """任务：内容提取"""
        logger.info(f"  [Task] extract_content: {file_path}")
        if _context.get('progress_callback'):
            _context['progress_callback']("extract", 0.1, "提取文档内容...")

        text_content, metadata = self._extract_content(file_path, document_id)

        if _context.get('progress_callback'):
            _context['progress_callback']("extract", 0.2, "内容提取完成")
    ...
    def _task_clean_text(self, text: str, metadata: dict, _context: dict) -> dict:
        """任务：数据清洗"""
        logger.info(f"  [Task] clean_text: {len(text)} 字符")
        if _context.get('progress_callback'):
            _context['progress_callback']("clean", 0.3, "清洗文本数据...")

        cleaned_text, clean_metadata = self._clean_text(text, metadata)

        if _context.get('progress_callback'):
            _context['progress_callback']("clean", 0.4, "数据清洗完成")
    ...
    def _task_extract_facts(
        ...
        """任务：提取事实语句"""
        logger.info(f"  [Task] extract_facts")
        if _context.get('progress_callback'):
            _context['progress_callback']("fact_extraction", 0.45, "提取结构化事实...")
        ...
        try:
            ...
            if _context.get('progress_callback'):
                _context['progress_callback']("fact_extraction", 0.5, f"提取了 {fact_count} 条事实")

            return {"fact_count": fact_count, "success": True}
        except Exception as e:
            logger.error(f"事实提取失败: {e}")
            return {"fact_count": 0, "success": False, "error": str(e)}

    def _task_chunk_document(self, cleaned_text: str, metadata: dict, _context: dict) -> list:
        """任务：文档切分"""
        logger.info(f"  [Task] chunk_document")
        if _context.get('progress_callback'):
            _context['progress_callback']("chunk", 0.5, "切分文档...")

        chunks = self.chunker.chunk_document(cleaned_text, metadata)

        if _context.get('progress_callback'):
            _context['progress_callback']("chunk", 0.6, f"切分完成：{len(chunks)} 个chunks")

        return chunks

    def _task_vectorize_chunks(self, chunks: list, _context: dict) -> list:
        """任务：向量化"""
        logger.info(f"  [Task] vectorize_chunks: {len(chunks)} chunks")
        if _context.get('progress_callback'):
            _context['progress_callback']("vectorize", 0.7, "向量化文本...")

        vectorized_chunks = self.vectorizer.vectorize_chunks(chunks)

        if _context.get('progress_callback'):
            _context['progress_callback']("vectorize", 0.8, "向量化完成")

        return vectorized_chunks

    def _task_store_chunks(
        self,
        ...
        """任务：存储到数据库"""
        logger.info(f"  [Task] store_chunks: {len(vectorized_chunks)} chunks")
        if _context.get('progress_callback'):
            _context['progress_callback']("index", 0.9, "存储到数据库...")
    ...
    def _task_build_knowledge_graph(
        self,
        cleaned_text: str,
        document_id: int,
        _context: dict
        ...
        """任务：构建知识图谱"""
        logger.info(f"  [Task] build_knowledge_graph")
        if _context.get('progress_callback'):
            _context['progress_callback']("knowledge_graph", 0.92, "构建知识图谱...")
        ...
        try:
            ...
            if _context.get('progress_callback'):
                _context['progress_callback']("knowledge_graph", 0.95, f"知识图谱构建完成: {len(entities)}个实体")
    ...
    def _task_finalize_document(
        ...
        """任务：更新文档状态"""
        logger.info(f"  [Task] finalize_document")
        ...
        if _context.get('progress_callback'):
            _context['progress_callback']("finalize", 1.0, "处理完成")

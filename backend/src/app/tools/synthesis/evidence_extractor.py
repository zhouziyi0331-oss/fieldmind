...
class EvidenceExtractor:
    ...
    def __init__(self, db: Session):
        self.db = db
        self.entity_service = create_engine()
        self.categorizer = get_entity_categorizer()
        self.vectorizer = UnifiedVectorizationEngine(
    ...
    def _get_document_chunks(self, document_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            # 使用统一引擎的ChromaDB客户端
            if not hasattr(self.vectorizer, 'chroma_client') or not self.vectorizer.chroma_client:
                logger.warning("ChromaDB未启用，无法获取chunks")
                return []

            collection = self.vectorizer.chroma_client.get_or_create_collection(
                name=self.vectorizer.chroma_collection_name
            )

        ...
        try:
            ...
            for doc in docs:
                if hasattr(doc, 'content_text') and doc.content_text:
                    all_content.append(doc.content_text[:5000])  # 限制每个文档5000字符
            ...
            if combined_content:
                # 使用新工具分析
                skill_result = analyze_with_skills(
                    content=combined_content,
                    enabled_skills=None,  # 使用所有Skills
                    report_format='summary',
                    include_statistics=True
                )

                # 转换为兼容格式
                return skill_result.get('skills_summary', {})

        except Exception as e:
            logger.warning(f"analyze_with_skills工具失败，使用备用方法: {e}")

        # Fallback: 从data_profile提取
        skill_results = {}

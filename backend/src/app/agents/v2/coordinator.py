...
from app.models.pipeline_state import PipelineExecution, PipelineStageResult
...
class AgentCoordinator:
    ...
    def process_project(
        ...
        pipeline_exec = PipelineExecution(
            ...
            metadata={'stages': [s.value for s in stages]}
        )
    ...
    def _update_execution_progress(
        ...
        pipeline_exec = db_session.query(PipelineExecution).filter(
    ...
    def _save_stage_result(
        ...
        """保存阶段结果到数据库"""
        stage_record = db_session.query(PipelineStageResult).filter(
            PipelineStageResult.execution_id == execution_id,
            PipelineStageResult.stage == stage.value
        ).first()
    ...
    def _find_unfinished_execution(
        ...
        """查找未完成的执行记录"""
        return db_session.query(PipelineExecution).filter(
    ...
    def _resume_execution(
        ...
        # 加载已完成的阶段结果
        stage_records = db_session.query(PipelineStageResult).filter(
            PipelineStageResult.execution_id == execution_id,
            PipelineStageResult.success == True
        ).all()
    ...
    def get_pipeline_status(
        ...
        # 查找最近的执行记录
        latest_exec = db_session.query(PipelineExecution).filter(
            PipelineExecution.project_id == project_id
        ).order_by(PipelineExecution.created_at.desc()).first()
        ...
        if latest_exec:
            # 从数据库读取详细状态
            stage_records = db_session.query(PipelineStageResult).filter(
                PipelineStageResult.execution_id == latest_exec.execution_id
            ).all()

    ...
    async def generate_synthesis_insights(
        ...
        if include_business_analysis:
            try:
                ...
                business_insights = {'error': str(e), 'executed_services': 0}
        ...
        if business_insights and 'results' in business_insights:
            ...
            if '04_market_demand' in service_results:
                ...
                key_insights.append({
                    ...
                    'service_id': '04',
                    'insight': f"预估年接待量: {demand.get('annual_visitors', {}).get('mid', 0)}人次",
                    'segments': demand.get('customer_segments', [])
                })

            # 从竞争对手分析(05)提取
            if '05_competitor' in service_results:
                competitor = service_results['05_competitor']
                uvp = competitor.get('unique_value_proposition', '')
                if uvp and uvp != '待深入调研后提炼':
                    key_insights.append({
                        'source': 'competitor_analysis',
                        'service_id': '05',
                        'insight': f"独特价值主张: {uvp}",
                        'differentiation': competitor.get('differentiation_opportunities', [])
                    })

            # 从商业格式分析(01)提取
            if '01_business_formats' in service_results:
                formats = service_results['01_business_formats']
                if 'existing_formats' in formats:
                    decision_factors.append({
                        'type': 'business_formats',
                        'service_id': '01',
                        'existing': formats.get('existing_formats', []),
                        'suggested': formats.get('suggested_formats', [])
                    })

            # 从创意文创分析(02)提取
            if '02_creative_analysis' in service_results:
                creative = service_results['02_creative_analysis']
                if 'creative_ideas' in creative:
            ...
            total = business_insights.get('total_services', 15)
            key_insights.append({
                'source': 'business_analysis_summary',
                'insight': f"商业分析服务执行: {executed}/{total}个服务完成",
                'failed_services': business_insights.get('failed_services', 0)
            })
